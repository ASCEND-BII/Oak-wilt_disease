...
jpeg("02-ts_a.jpeg", quality = 100, res = 600, width = 90, height = 50, units = "mm", pointsize = 12) # JPEG device
...
ggplot() +
      ...
      geom_point(data = data_TSS, aes(x = TSS_date, TSS/10000), col = "white", fill = "grey", shape = 21, size = 1) +
      ...
      scale_y_continuous(expand = c(0,0),
                         limits = c(-0.35, 0.5)) +
      ylab("CCI") + xlab("Year") +
      annotate("text", label = "Healthy", x = as.IDate("2017-07-27"), y = -0.26, size = 1, colour = pa[1]) +
      annotate("text", label = "Healthy", x = as.IDate("2018-07-27"), y = -0.26, size = 1, colour = pa[1]) +
      annotate("text", label = "Symptomatic", x = as.IDate("2019-08-09"), y = -0.26, size = 1, colour = pa[2]) +
      annotate("text", label = "Dead", x = as.IDate("2020-07-20"), y = -0.26, size = 1, colour = pa[3]) +
      annotate("text", label = "Dead", x = as.IDate("2021-07-27"), y = -0.26, size = 1, colour = pa[3]) +
      th
