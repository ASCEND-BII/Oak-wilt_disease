...
#Plot
plot <- ggplot() +
  geom_vline(data = summary_gruops, aes(xintercept = mean(value)), linetype = "dotted", 
             color = "black", size = 0.4) +
  geom_errorbar(data = summary_condition, aes(xmin= value-sd, xmax= value+sd, 
                                              y = Year, colour = Condition, 
                                              group = Condition), width=.2,
                position=position_dodge(0.1)) +
  geom_point(data = summary_condition, aes(x = value, y = Year, colour = Condition, 
                                           group = Condition, shape = Condition), 
             size = 3, position=position_dodge(0.1)) +
  geom_path(data = summary_condition, aes(x = value, y = Year, group = Condition, 
