...
get_CRE <- function(root_path, out_path, threads = 26) {
  
  #Get scenes to work with
  frame <- path_vi_scenes(root_path)
  
  #Get just RED and GRN
  frame <- subset(frame, VI == "RE3" |  VI == "RE1" )
  ...
  foreach(i = 1:n_scenes,
          ...
          .inorder = F) %dopar% {
            ...
            #Files
            GREEN <- frame[VI == "RE3" & unique == i]
            RED <- frame[VI == "RE1" & unique == i]
            ...
            #Get index
            CRE <- round(((rGREEN / rRED) - 1) * 1000, 0)
            
            #export name
            export_name <- strsplit(GREEN$scene[1], "RE3")[[1]]
            export_name <- paste0(out_path, "/", 
                                  GREEN$tile[1], "/", 
                                  export_name[1], "CRE", export_name[2])
            ...
            #Remove residuals
            rm(list = c("GREEN", "RED", "rGREEN", "rRED", "CRE", "export_name"))
            gc()
...
get_CRE(root_path, out_path, threads)
